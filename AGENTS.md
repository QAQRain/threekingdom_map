...
- [x] **常駐資源面板**（取代彈出視窗）：地圖左上常駐顯示所選據點的兵裝材料（`index.html` 的 `#detailPanel`）。資源顯示在最上方（金框強調），下方依序為：數量欄（預設 1，材料 × 數量）→ 合計所需 → 我的資源（皮/鐵/韌皮/精鐵，存 localStorage key `sanguo_player_res`，含「顯示不足警示」開關 `resToggle`，**開啟才顯示紅字**）。先點兵團再點據點時，面板只顯示該兵團（app.js `renderPanel`）。**資源圖示**：`assets/resources/{pi,tie,renpi,jingtie}.png`，材料以圖示＋數量顯示，順序固定「皮/鐵 上層、韌皮/精鐵 下層」
...
- 一律使用繁體中文回覆。
- **更新流程**：使用者要求「更新」後，AI 應直接 commit + push，不需再逐次詢問（用 `.git-credentials` 的 token 一次性內嵌 URL）。⚠ 仍只能 add 指定檔案，不能 `add -A`。
