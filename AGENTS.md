...
- [x] **常駐資源面板**（取代彈出視窗）：地圖左上常駐顯示所選據點的兵裝材料（`index.html` 的 `#detailPanel`）。資源顯示在最上方（金框強調），下方依序為：數量欄（預設 1，材料 × 數量）→ 合計所需 → 我的資源（皮/鐵/韌皮/精鐵，存 localStorage key `sanguo_player_res`，含「顯示不足警示」開關 `resToggle`，**開啟才顯示紅字**）。先點兵團再點據點時，面板只顯示該兵團（app.js `renderPanel`）
